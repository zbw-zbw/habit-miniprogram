...
function analyzeHabitPatterns(habit, checkins) {
    const completedCheckins = checkins.filter(c => c.isCompleted);
    // 按时间排序
    const sortedCheckins = [...completedCheckins].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    // 分析连续性
 ...
 * @returns 连续性分析
 */
function analyzeStreaks(sortedCheckins) {
    ...
    for (let i = 1; i < sortedCheckins.length; i++) {
        ...
        // 检查是否为连续日期
        const dayDiff = (0, date_1.daysBetween)(prevDate, currDate);
        if (dayDiff === 1) {
            // 连续
            currentStreak++;
 ...
 * @returns 周期性分析
 */
function analyzePeriodicity(sortedCheckins) {
 ...
 * @returns 时间分布分析
 */
function analyzeTimeDistribution(checkins) {
 ...
 * @returns 科学建议
 */
function generateRecommendations(habit, analysisResults) {
