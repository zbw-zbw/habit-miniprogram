...
class AchievementService {
  ...
  async updateProgress(condition: AchievementCondition, value: number) {
    ...
    switch (condition.type) {
      case 'streak':
        // 更新连续打卡相关的成就
        updatedAchievements = await this.updateStreakAchievements(
          condition,
          value
        );
        break;
      case 'count':
        // 更新计数相关的成就
        updatedAchievements = await this.updateCountAchievements(
          condition,
          value
        );
        break;
      case 'duration':
        // 更新时长相关的成就
        updatedAchievements = await this.updateDurationAchievements(
          condition,
          value
        );
        break;
   ...
   * 更新连续打卡相关的成就
   */
  private async updateStreakAchievements(
   ...
   * 更新计数相关的成就
   */
  private async updateCountAchievements(
    condition: AchievementCondition,
    value: number
  ): Promise<IAchievement[]> {
    const updatedAchievements: IAchievement[] = [];
   ...
   * 更新时长相关的成就
   */
  private async updateDurationAchievements(
    condition: AchievementCondition,
    value: number
  ): Promise<IAchievement[]> {
    const updatedAchievements: IAchievement[] = [];
