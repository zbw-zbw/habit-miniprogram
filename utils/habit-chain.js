...
exports.recommendHabitChains = exports.generateHabitChains = exports.analyzeHabitRelation = exports.RelationStrength = exports.HabitRelationType = void 0;
// 习惯关联类型
var HabitRelationType;
(function (HabitRelationType) {
    HabitRelationType["TRIGGER"] = "trigger";
    HabitRelationType["SEQUENTIAL"] = "sequential";
    HabitRelationType["COMPLEMENTARY"] = "complementary";
    HabitRelationType["CONFLICTING"] = "conflicting";
})(HabitRelationType = exports.HabitRelationType || (exports.HabitRelationType = {}));
// 习惯关联强度
var RelationStrength;
(function (RelationStrength) {
    RelationStrength["WEAK"] = "weak";
    RelationStrength["MEDIUM"] = "medium";
    RelationStrength["STRONG"] = "strong";
})(RelationStrength = exports.RelationStrength || (exports.RelationStrength = {}));
/**
 * 分析两个习惯之间的关联
 ...
 * @param checkins 打卡记录
 */
function analyzeHabitRelation(sourceHabit, targetHabit, checkins) {
    // 按习惯ID分组打卡记录
...
exports.analyzeHabitRelation = analyzeHabitRelation;
/**
 * 分析两个习惯的打卡时间关系
 */
function analyzeTimeRelation(sourceCheckins, targetCheckins) {
    // 找出同一天的打卡记录
    const sameDayCheckins = [];
    ...
    // 计算时间差
    const timeDiffs = sameDayCheckins.map(pair => {
 ...
 * 分析两个习惯的共现关系
 */
function analyzeCooccurrence(sourceCheckins, targetCheckins) {
    // 所有不重复的日期
 ...
 */
function generateHabitChains(habits, relations) {
    const chains = [];
    // 按关联强度降序排序
...
exports.generateHabitChains = generateHabitChains;
/**
 * 查找触发型习惯链
 */
function findTriggerChains(habits, relations) {
    const chains = [];
    // 找出所有触发关系
    const triggerRelations = relations.filter(r => r.type === HabitRelationType.TRIGGER &&
 ...
 * 构建习惯关系图
 */
function buildHabitGraph(habits, relations) {
    const graph = new Map();
    // 初始化所有习惯节点
    habits.forEach(habit => {
        graph.set(habit.id, []);
 ...
 * 深度优先搜索查找习惯路径
 */
function findPaths(habitId, graph, visited, path, chains, habits, usedHabits) {
    visited.add(habitId);
    path.push(habitId);
    // 当路径长度大于等于3时，创建一个习惯链
    if (path.length >= 3 && !isSubsetOfExistingChain(path, chains)) {
        const chainHabits = path.map(id => habits.find(h => h.id === id));
        const validHabits = chainHabits.filter(Boolean);
 ...
 * 查找是否已经是现有链的子集
 */
function isSubsetOfExistingChain(path, chains) {
 ...
 * 查找互补型习惯链
 */
function findComplementaryChains(habits, relations) {
    const chains = [];
    // 找出所有互补关系
    const complementaryRelations = relations.filter(r => r.type === HabitRelationType.COMPLEMENTARY &&
        r.strength !== RelationStrength.WEAK &&
        r.confidence > 0.5);
    // 按照源习惯分组
    ...
    complementaryRelations.forEach(relation => {
        ...
        const group = habitGroups.get(sourceId);
 ...
 */
async function recommendHabitChains(habits, checkins) {
    // 分析所有习惯间的关系
    const relations = [];
...
exports.recommendHabitChains = recommendHabitChains;
