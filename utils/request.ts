...
// 显示加载提示
const showLoadingToast = (text: string = '加载中...') => {
  wx.showLoading({
    title: text,
    mask: true
  });
 ...
 * @returns 处理后的响应数据
 */
const responseInterceptor = <T>(response: RequestResponse<ApiResponse<T>>): Promise<T> => {
...
export const request = <T>(options: RequestOptions): Promise<T> => {
  ...
  return new Promise<T>((resolve, reject) => {
    wx.request({
      ...
      timeout,
      success: (res: RequestResponse<ApiResponse<T>>) => {
        try {
