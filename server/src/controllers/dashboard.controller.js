...
exports.getDashboard = async (req, res) => {
  try {
    ...
    const userStreakData = await User.findById(userId).select('currentStreak longestStreak');
    ...
    const habitStatsPromises = activeHabits.map(async habit => {
      ...
      // 计算习惯完成率
      const completionRate = totalCheckins > 0 ? (totalCompleted / totalCheckins) * 100 : 0;
      ...
      habitStats[habitId] = {
        ...
        lastCompletedDate: lastCompletedCheckin ? lastCompletedCheckin.date : null
      };
    });
...
exports.getAllHabits = async (req, res) => {
  try {
    ...
    const shouldIncludeCheckins = includeCheckins === 'true' || includeCheckins === true;
...
exports.getAnalytics = async (req, res) => {
  try {
    ...
    const thisWeekCheckins = checkins.filter(c => 
      c.date >= oneWeekAgoFormatted && c.date <= endDateFormatted && c.isCompleted
    ).length;
    ...
    // 7. 计算周趋势
    const weeklyTrend = lastWeekCheckins > 0 ? 
    ...
    const habitStatsPromises = habitsWithStats.map(async habit => {
      ...
      // 获取习惯的完成记录
      const habitCheckins = checkins.filter(c => c.habit._id.toString() === habitId);
      const totalHabitCheckins = habitCheckins.length;
      const completedHabitCheckins = habitCheckins.filter(c => c.isCompleted).length;
      
      // 计算习惯完成率
      const completionRate = totalHabitCheckins > 0 ? 
        (completedHabitCheckins / totalHabitCheckins) * 100 : 0;
    ...
    activeHabits.forEach(habit => {
      ...
      if (habitStats[habitId]) {
        categoryData[category].totalStreak += habitStats[habitId].currentStreak || 0;
        categoryData[category].completed += habitStats[habitId].totalCompletions || 0;
      }
    });
    ...
    // 11. 计算平均完成率
    const averageCompletionRate = totalCheckins > 0 ? 
      (completedCheckins / totalCheckins) * 100 : 0;
    ...
    dateRange.forEach(date => {
      ...
      const completionRate = totalHabitsForDay > 0 ? 
        (completedHabitsForDay / totalHabitsForDay) * 100 : 0;
