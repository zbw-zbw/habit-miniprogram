...
Component({
    ...
    methods: {
        ...
        prepareData() {
            ...
            if (!chain || !habits || habits.length === 0)
                return;
            const nodes = [];
            const edges = [];
            // 计算节点布局
            const chainHabits = chain.habits.sort((a, b) => a.order - b.order);
            ...
            // 创建节点
            chainHabits.forEach((item, index) => {
                const habitInfo = habits.find((h) => h.id === item.habitId);
                if (!habitInfo)
                    return;
         ...
         */
        onTouchMove(e) {
            if (!this.data.editable || !this.data.isDragging || this.data.draggedNodeIndex === -1)
                return;
            const touch = e.touches[0];
        ...
        toggleOptional(habitId) {
            ...
            const habitIndex = newChain.habits.findIndex((h) => h.habitId === habitId);
            if (habitIndex !== -1) {
         ...
         * 更改节点顺序
         */
        changeNodeOrder(habitId, newOrder) {
            if (!this.data.editable)
                return;
            ...
            // 更新所有受影响的习惯顺序
            newChain.habits.forEach((h, i) => {
                if (h.habitId === habitId) {
                    h.order = newOrder;
                }
                else if ((newOrder > currentOrder && h.order > currentOrder && h.order <= newOrder) ||
