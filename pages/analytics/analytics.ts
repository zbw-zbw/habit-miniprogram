 ...
 * @returns 格式化后的日期字符串
 */
function formatDateCustom(date: Date, format = 'YYYY-MM-DD'): string {
...
interface IPageData {
  ...
  habitsMap: Record<string, IHabit>;
  chartData: {
    dates: string[];
    values: number[];
...
Page<IPageData, IPageMethods>({
  ...
  processAnalyticsData(data: any) {
    ...
    const habits: IHabit[] = [];
    const habitsMap: Record<string, IHabit> = {};
    ...
    if (habitStats && typeof habitStats === 'object') {
      Object.keys(habitStats).forEach((key) => {
        ...
        if (statData && statData.habit) {
          const habit = statData.habit as IHabit;
          habits.push(habit);
          // 使用一致的ID字段
    ...
    if (habits.length > 0 && currentStreak === 0) {
      ...
      habits.forEach((habit) => {
        // 使用类型断言和可选链处理stats属性
        const habitStats = (habit as any).stats;
    ...
    const completionRate = Number(
      summary.averageCompletionRate || todayCompletionRate || 0
    );
  ...
  calculateHabitStats(habits: IHabit[], checkins: ICheckin[]) {
    ...
    Object.entries(this.data.habitsMap).forEach(([habitId, habit]) => {
      ...
      const stats = this.data.habitStats
        ? (this.data.habitStats as any)[habitId]
        : null;
  ...
  generateChartData(timelineData: any[]) {
    ...
    if (this.data.timeRange === 'year') {
      ...
      const dates = timelineData.map((item) =>
        formatDateCustom(new Date(item.date), 'MM-dd')
      );
   ...
   * 为年视图生成图表数据
   */
  generateYearChartData(timelineData: any[]) {
    // 按月份分组数据
    ...
    const completionRates = allMonths.map((month) => {
      if (monthlyData[month] && monthlyData[month].count > 0) {
        // 计算该月的平均完成率
        return Math.round(
          monthlyData[month].completionRate / monthlyData[month].count
        );
      }
      return 0; // 没有数据则返回0
    });
    ...
    try {
      ...
      new wxCharts({
        ...
        yAxis: {
          ...
          fontColor: '#909399', // 设置Y轴标签颜色
          titleFontColor: '#909399', // 设置Y轴标题颜色
          disabled: false, // 启用Y轴
          gridColor: '#E4E7ED', // 网格线颜色
          titleFontSize: 10, // Y轴标题字体大小
          fontWeight: 'normal', // 字体粗细
          padding: 30, // 显著增加内边距，使数据与Y轴标签保持足够距离
          splitNumber: 5, // 将Y轴分为5个等分
        },
        xAxis: {
          fontColor: '#909399',
          rotate: chartConfig.xAxisRotate,
          itemCount: timeRange === 'year' ? 12 : timeRange === 'month' ? 8 : 7, // 控制X轴显示的项目数量
          boundaryGap: true, // 在X轴两端留出间隙
          disableGrid: true, // 禁用X轴网格线以减少视觉干扰
        },
        width: chartConfig.width,
        height: chartConfig.height,
        dataLabel: timeRange === 'week', // 只在周视图显示数据标签
        legend: false,
        extra: {
          lineStyle: 'curve', // 使用曲线
          yAxis: {
            gridType: 'dash', // 使用虚线网格
            dashLength: 4, // 虚线长度
          },
          point: {
        ...
        try {
          new wxCharts({
            ...
            yAxis: {
              ...
              gridColor: '#E4E7ED', // 网格线颜色
              fontWeight: 'normal', // 字体粗细
              padding: 30, // 增加内边距，使数据与Y轴标签保持足够距离
            ...
            xAxis: {
              ...
              rotate: chartConfig.xAxisRotate,
              itemCount:
                timeRange === 'year' ? 12 : timeRange === 'month' ? 8 : 7, // 控制X轴显示的项目数量
              boundaryGap: true, // 在X轴两端留出间隙
              disableGrid: true, // 禁用X轴网格线以减少视觉干扰
            },
            width: chartConfig.width,
            ...
            extra: {
              column: {
                width:
                  timeRange === 'year' ? 20 : timeRange === 'month' ? 10 : 15, // 根据时间范围调整柱形宽度
              },
              yAxis: {
                gridType: 'dash', // 使用虚线网格
                dashLength: 4, // 虚线长度
   ...
   */
  calculateYAxisStep(maxValue: number): number {
    if (maxValue <= 5) return 1; // 如果最大值小于等于5，步长为1
    if (maxValue <= 10) return 2; // 如果最大值小于等于10，步长为2
    if (maxValue <= 20) return 4; // 如果最大值小于等于20，步长为4
    if (maxValue <= 50) return 10; // 如果最大值小于等于50，步长为10
