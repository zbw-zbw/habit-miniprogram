...
interface IPageMethods {
  loadData(): void;
  generateInsights(habits: IHabit[], checkins: ICheckin[]): void;
  findBestHabit(habits: IHabit[], checkins: ICheckin[]): void;
  findHabitNeedsImprovement(habits: IHabit[], checkins: ICheckin[]): void;
  analyzePatterns(habits: IHabit[], checkins: ICheckin[]): void;
  generateRecommendations(habits: IHabit[], checkins: ICheckin[]): void;
...
Page<IPageData, IPageMethods>({
   ...
   * 生成洞察
   */
  generateInsights(habits: IHabit[], checkins: ICheckin[]) {
    const insights = [];
    ...
    habits.filter(h => !h.isArchived).forEach(habit => {
      ...
      const totalDays = getDaysBetween(new Date(habit.createdAt), today);
   ...
   * 找出最好的习惯
   */
  findBestHabit(habits: IHabit[], checkins: ICheckin[]) {
    if (habits.length === 0) {
      return;
    }
   ...
   * 找出需要改进的习惯
   */
  findHabitNeedsImprovement(habits: IHabit[], checkins: ICheckin[]) {
    if (habits.length === 0) {
      return;
    }
    ...
    habits.filter(h => !h.isArchived).forEach(habit => {
      ...
      // 找出最后一次完成的日期
      const sortedCheckins = [...habitCheckins].sort((a, b) => 
   ...
   * 分析模式
   */
  analyzePatterns(habits: IHabit[], checkins: ICheckin[]) {
   ...
   * 生成建议
   */
  generateRecommendations(habits: IHabit[], checkins: ICheckin[]) {
