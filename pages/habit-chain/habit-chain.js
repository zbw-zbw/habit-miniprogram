...
const habit_chain_1 = require("../../utils/habit-chain");
// 获取应用实例
const app = getApp();
Page({
    /**
        ...
        try {
            ...
            for (let i = 0; i < habits.length; i++) {
                for (let j = 0; j < habits.length; j++) {
                    if (i !== j) {
                        const relation = (0, habit_chain_1.analyzeHabitRelation)(habits[i], habits[j], checkins);
                        relations.push(relation);
                    }
                }
            }
            // 过滤出强关联
            const strongRelations = relations.filter(r => r.strength !== habit_chain_1.RelationStrength.WEAK &&
                r.confidence > 0.5);
            // 添加习惯名称
            ...
            if (strongRelations.length > 0) {
                const triggerCount = strongRelations.filter(r => r.type === habit_chain_1.HabitRelationType.TRIGGER).length;
                const complementaryCount = strongRelations.filter(r => r.type === habit_chain_1.HabitRelationType.COMPLEMENTARY).length;
                const sequentialCount = strongRelations.filter(r => r.type === habit_chain_1.HabitRelationType.SEQUENTIAL).length;
                const conflictingCount = strongRelations.filter(r => r.type === habit_chain_1.HabitRelationType.CONFLICTING).length;
     ...
     * 生成推荐习惯链
     */
    async generateRecommendedChains(habits, checkins) {
        // 使用习惯链推荐函数
        const chains = await (0, habit_chain_1.recommendHabitChains)(habits, checkins);
        // 为习惯链添加习惯详情
