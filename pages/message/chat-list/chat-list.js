...
const message_api_1 = require("../../../services/api/message-api");
Page({
    data: {
        // 登录状态
        hasLogin: false,
    ...
    loadChatSessions() {
        ...
        // 调用API获取聊天会话列表
        if (message_api_1.messageAPI.getChatSessions) {
            message_api_1.messageAPI.getChatSessions()
                .then(sessions => {
                // 处理会话数据
                const processedSessions = sessions.map(session => ({
                    ...session,
                    avatar: (0, image_1.getFullImageUrl)(session.avatar || '/assets/images/default-avatar.png'),
    ...
    onSearch() {
        ...
        // 在现有会话中搜索
        const results = this.data.sessions.filter(session => session.nickname.includes(searchValue) ||
