...
Page({
  ...
  onTabChange(e: any) {
    ...
    const tab = tabMapping[index] as TabType;
    ...
    communityAPI
      ...
      .then((results) => {
        if (activeTab === 'all') {
          ...
          const processedGroups = (results.groups || []).map((group) => {
            ...
            const isCreator =
              ...
              (group.creator._id === currentUserId ||
                (group.creator as any).id === currentUserId);
          ...
          const processedChallenges = (results.challenges || []).map(
            (challenge) => {
              // 获取当前用户ID
              const app = getApp<IAppOption>();
              const currentUserId = app?.globalData?.userInfo?.id;
              ...
              const isCreator =
                ...
                (challenge.creator._id === currentUserId ||
                  (challenge.creator as any).id === currentUserId);
      ...
      communityAPI
        ...
        .then(() => {
          ...
          this.setData({
            ...
            // 增加成员数
            [`groupResults[${index}].membersCount`]:
        ...
        .catch((error) => {
          ...
          if (
            (error && error.message && error.message.includes('已加入')) ||
      ...
      communityAPI
        ...
        .then(() => {
          ...
          this.setData({
            ...
            // 减少成员数，确保不小于0
            [`groupResults[${index}].membersCount`]: Math.max(
              0,
              (group.membersCount || 0) - 1
            ),
          });
      ...
      communityAPI
        ...
        .then(() => {
          ...
          this.setData({
            ...
            // 增加参与人数
            [`challengeResults[${index}].participants`]:
              (challenge.participants || 0) + 1,
            [`challengeResults[${index}].participantsCount`]:
      ...
      communityAPI
        ...
        .then(() => {
          this.setData({
            ...
            [`challengeResults[${index}].participants`]: Math.max(
              1,
              (challenge.participants || 0) - 1
            ),
            [`challengeResults[${index}].participantsCount`]: Math.max(
              1,
              (challenge.participantsCount || challenge.participants || 0) - 1
            ),
          });
