...
Page({
    ...
    toggleJoinChallenge() {
        ...
        const apiCall = isJoined
            ? api_1.communityAPI.leaveChallenge(challengeId)
    ...
    viewAllParticipants() {
        ...
        wx.navigateTo({
            url: `/pages/community/challenges/participants/participants?id=${challengeId}&name=${encodeURIComponent(((_a = this.data.challenge) === null || _a === void 0 ? void 0 : _a.name) || '挑战')}`
        });
    },
    /**
     * 查看排行榜
    ...
    viewLeaderboard() {
        ...
        wx.navigateTo({
            url: `/pages/community/challenges/leaderboard/leaderboard?id=${challengeId}&name=${encodeURIComponent(((_a = this.data.challenge) === null || _a === void 0 ? void 0 : _a.name) || '挑战')}`
        });
    },
    /**
     * 分享挑战
        ...
        wx.showModal({
            ...
            success: (res) => {
                if (res.confirm) {
                    ...
                    // 调用API解散挑战
                    api_1.communityAPI.dismissChallenge(challengeId)
                        .then(() => {
                        wx.showToast({
                            title: '挑战已解散',
                            icon: 'success'
