...
Page({
    ...
    async runAllTests() {
        ...
        await test_utils_1.TestManager.runAllTests();
    ...
    async testAPIAvailability() {
        this.clearTestResults();
        this.setData({ testing: true });
        await test_utils_1.TestManager.testAPIAvailability();
        this.setData({ testing: false });
    },
    /**
     * 测试数据流程
    ...
    async testDataFlow() {
        this.clearTestResults();
        this.setData({ testing: true });
        await test_utils_1.TestManager.testDataFlow();
        this.setData({ testing: false });
    },
    /**
     * 测试离线功能
    ...
    testOfflineFeatures() {
        this.clearTestResults();
        this.setData({ testing: true });
        test_utils_1.TestManager.testOfflineFeatures();
        this.setData({ testing: false });
    },
    /**
     * 测试同步管理器
    ...
    testSyncManager() {
        this.clearTestResults();
        this.setData({ testing: true });
        test_utils_1.TestManager.testSyncManager();
        setTimeout(() => {
            this.setData({ testing: false });
        }, 3000);
    },
