...
Page({
    ...
    loadDashboardData() {
        ...
        // 使用API获取仪表盘数据
        api_1.analyticsAPI.getDashboard()
            .then(dashboardData => {
    ...
    loadChartData() {
        ...
        // 使用API获取完成率数据
        api_1.analyticsAPI.getCompletionRate(params)
            .then(completionData => {
    ...
    processChartData(data, timeRange) {
        ...
        if (data && data.data) {
            ...
            // 提取日期和完成率数据
            chartData.weekLabels = data.data.map((item) => this.formatDate(item.date, dateFormat));
            chartData.weeklyCompletion = data.data.map((item) => item.rate);
        }
        return chartData;
    },
    /**
    ...
    loadHabits() {
        ...
        api_1.habitAPI.getHabits()
            ...
            const statsPromises = habits.map(habit => {
                ...
                return api_1.analyticsAPI.getHabitStats({ habitId });
            });
            // 等待所有统计数据获取完成
